...
from utils_ast import convert_examples_to_features_ast
...
def convert_examples_to_features(js,tokenizer,args):
    ...
    if args.ast_type == "abstract":
        ...
        code=' '.join(flatten_ast_str.split())
    else:
        flatten_ast_details_str = ' '.join([str(item) for item in flatten_ast_details])
        code=' '.join(flatten_ast_details_str.split())

    code_tokens=tokenizer.tokenize(code)[:args.block_size-4]
    source_tokens =[tokenizer.cls_token] + code_tokens + [tokenizer.dia_token] + [tokenizer.dib_token] + [tokenizer.sep_token]
    source_ids =  tokenizer.convert_tokens_to_ids(source_tokens)
    padding_length = args.block_size - len(source_ids)
    
    source_ids+=[tokenizer.pad_token_id]*padding_length
    att_masks = (np.array(source_ids).__ne__(tokenizer.pad_token_id)*1).tolist()

    return InputFeatures(source_tokens,source_ids,att_masks,js['idx'],js['target'])
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):       
        return torch.tensor(self.examples[i].input_ids), torch.tensor(self.examples[i].label), torch.tensor(self.examples[i].att_masks)
...
def evaluate(args, model, tokenizer,eval_when_training=False):
    ...
    for batch in eval_dataloader:
        ...
        masks=batch[2].to(args.device) 
        with torch.no_grad():
            lm_loss,logit = model(inputs,masks,label)
    ...
    """
    with one neuron: preds=logits[:,0]>0.5
    """
    ...
    result = {
        ...
        "eval_f1":round(src_test_f1_binary,4),
    }
    return result
...
def test(args, model, tokenizer):
    ...
    result = {
        ...
        "test_f1_binary": round(src_test_f1_binary, 4),
    }
    return result
...
def main():
    ...
    # devign vs regvd vs textcnn vs bilstm vs roberta
    parser.add_argument("--model", default="roberta", type=str,help="")
    ...
    if args.model == "gvd":
        model = GNNGVD(model, config, tokenizer, args)
    else:
        print("Please set the model argument to 'gvd'!!!")
