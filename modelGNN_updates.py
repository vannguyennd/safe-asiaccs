...
class RGGNN(nn.Module):
    ...
    def forward(self, inputs, adj, mask):
        ...
        for idx_layer in range(self.num_GNN_layers):
            if self.residual:
                x = self.alpha_weight * x + self.gatedGNN(x.double(), adj.double()) * mask.double()  # add residual connection
            else:
    ...
    def forward(self, inputs, adj, mask):
        ...
        for idx_layer in range(self.num_GNN_layers):
            if idx_layer == 0:
                ...
                if self.residual:
                    x = self.alpha_weight * x + self.gnnlayers[idx_layer](x, adj) * mask  # Residual Connection
                else:
                    x = self.gnnlayers[idx_layer](x, adj) * mask
        # soft attention
